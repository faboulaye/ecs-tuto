...
todo.install:
	...
	cd backend/todo-app && $(POETRY) install --without dev,test

todo.install-test-dependencies:
	$(info [*] Install test dependencies)
	cd backend/todo-app && $(POETRY) install --with dev,test

todo.test:
	$(info [*] Build todo app tag docker container)
	cd bacnkend/todo-app && $(POETRY) run pytest
...
todo.run:
	...
	cd backend/todo-app && $(POETRY) run $(UVICORN) src.app:api --host 0.0.0.0 --port 8080 --reload
...
# ECS Tuto Infrastructure
infra.deploy: ## Deploy ECS Tuto infrastructure
	$(info ${BLUE}[*] Deploy ECS Tuto infrastructure ...${RESET})
	$(SAM) package --config-env infra
	$(SAM) deploy --config-env infra --no-fail-on-empty-changeset